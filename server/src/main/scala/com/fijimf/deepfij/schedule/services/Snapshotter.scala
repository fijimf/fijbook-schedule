...
import com.fijimf.deepfij.schedule.model.Dao.{AliasDao, ConferenceDao, TeamDao}
...
class Snapshotter[F[_]](xa: Transactor[F])(implicit F: Sync[F]) {
  ...
  def insertTeams(teams: List[TeamRecord]): F[Map[String, Team]] = {
    for {
      ts <- teams.map(tr => TeamDao.insert(tr.toTeam).withUniqueGeneratedKeys[Team](TeamDao.cols: _*).transact(xa)).sequence
    } yield {
  ...
  def insertAliases(aliases: List[AliasRecord], teamMap: Map[String, Team]): F[List[Alias]] = {
    for {
      ...
      }).map(a => AliasDao.insert(a).withUniqueGeneratedKeys[Alias](AliasDao.cols: _*).transact(xa)).sequence
    } yield {
      as
    }
  }
  ...
  def insertConferences(conferences: List[ConferenceRecord]): F[List[Conference]] = {
    for {
      cs <- conferences.map(cr => ConferenceDao.insert(cr.toConference).withUniqueGeneratedKeys[Conference](ConferenceDao.cols: _*).transact(xa)).sequence
    } yield {
      cs
    }
  }
    ...
    s3is.use(is => {
      ...
      value match {
        ...
        case Right(data: StaticScheduleData) =>
          for {
            _ <- TeamDao.truncate().run.transact(xa)
            _ <- AliasDao.truncate().run.transact(xa)
            _ <- ConferenceDao.truncate().run.transact(xa)
